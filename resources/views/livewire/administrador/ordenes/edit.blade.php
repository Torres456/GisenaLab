    ...
    <div class="flex w-full dark:bg-slate-800 rounded p-5">
        <form wire:submit="new_register" class="grid md:grid-cols-2 gap-3 w-full">
            <div class="flex flex-col">
                ...
                <x-select wire:model="newRegister.prioridad" class="w-full">
                    <option value="">Seleccione una opción</option>
                    <option value="0">Normal</option>
                    <option value="1">Urgente</option>
                </x-select>
                <x-input-error for="newRegister.prioridad" />
            </div>

            <div class="flex flex-col">
                <label for="nombre">Requiere Factura:<span class="text-red-600">*</span></label>
                <x-select wire:model="newRegister.factura" class="w-full">
                    <option value="">Seleccione una opción</option>
                    <option value="0">No</option>
                    <option value="1">Si</option>
                </x-select>
                <x-input-error for="newRegister.factura" />
            </div>

            <div class="flex flex-col md:col-span-2">
                <label for="nombre">Información adicional:</label>
                <x-textarea wire:model="newRegister.informacion" class="w-full">
                </x-textarea>
                <x-input-error for="newRegister.informacion" />
            </div>

            <div class="flex flex-col">
                <label for="nombre">Cliente:<span class="text-red-600">*</span></label>
                <x-select wire:model.live="newRegister.cliente" class="w-full">
                    <option value="">Seleccione una opción</option>
                    @foreach ($clientes as $cliente)
                        <option value="{{ $cliente->id_cliente }}">{{ $cliente->razon_social }}</option>
                    @endforeach
                </x-select>
                <x-input-error for="newRegister.cliente" />
            </div>

            <div class="flex flex-col">
                <label for="nombre">Interesado:<span class="text-red-600">*</span></label>
                <x-select wire:model="newRegister.interesado" class="w-full">
                    <option value="">Seleccione una opción</option>
                ...
                <x-input-error for="newRegister.interesado" />
            </div>

            <div class="flex flex-col md:col-span-2">
                <label for="nombre">Remisión de muestra:</label>
                <x-input wire:model="newRegister.remision" class="w-full" />
                <x-input-error for="newRegister.remision" />
            </div>

            <div class="flex flex-col ">
                <label for="nombre">Tipo de documento:</label>
                <x-input wire:model="newRegister.documento" class="w-full" />
                <x-input-error for="newRegister.documento" />
            </div>
            <div class="flex flex-col ">
                <label for="nombre">Folio de docuemto:</label>
                <x-input wire:model="newRegister.folio" class="w-full" />
                <x-input-error for="newRegister.folio" />
            </div>

            <div class="flex flex-col md:col-span-2">
                <label for="nombre">NETSUITE:</label>
                <x-input wire:model="newRegister.netsuite" class="w-full" />
                <x-input-error for="newRegister.netsuite" />
            </div>
