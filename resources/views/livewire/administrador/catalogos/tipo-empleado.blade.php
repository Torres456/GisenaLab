    ...
    <div class="flex gap-3 items-center dark:text-white mb-5 max-md:flex-col">
        <div class="flex flex-row w-full gap-2">
            ...
            <div class="flex flex-col w-full">
                ...
                <x-input wire:model.live="search" placeholder="(Nombre del rol)" class="w-full" />
            </div>
        </div>
                    ...
                    @foreach ($roles as $rol)
                        <tr
                            ...
                            <th scope="row"
                                ...
                                {{ $rol->id_tipo_empleado }}
                            </th>
                            <td class="px-6 py-4 text-center">
                                {{ $rol->descripcion_puesto }}
                            </td>
                            <td class="px-6 py-4 text-center">
                                <x-button wire:click="edit_register({{ $rol->id_tipo_empleado }})">
                            ...
                            <x-td>
                                @if ($rol->estatus == 1)
                                    <x-button wire:click="estatus_register({{  $rol->id_tipo_empleado  }})">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round"
                                ...
                                @else
                                    <x-danger-button wire:click="estatus_register({{  $rol->id_tipo_empleado  }})">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round"
