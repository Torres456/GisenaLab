<div>
    ...
    <div class="flex gap-3 items-center dark:text-white mb-5 max-md:flex-col">
        <div class="flex flex-row w-full gap-2 max-md:flex-col">
            ...
            <div class="flex flex-col w-full">
                ...
                <x-input wire:model.live="search" placeholder="(Datos de muestra o por laboratorios)" class="w-full" />
            </div>
        </div>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            @if ($count != 0)
                <tbody>
                    @foreach ($datos_muestras as $datos_muestra)
                        <tr
                            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <th scope="row"
                                class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white text-center">
                                {{ $datos_muestra->id_descrip_muestra }}
                            </th>
                            <td class="px-6 py-4 text-center">
                                {{ $datos_muestra->nombre_descrip }}
                            </td>
                            <td class="px-6 py-4 text-center">
                                {{ $datos_muestra->tipo_muestra->nom_tipo_muestra }}
                            </td>
                            <td class="px-6 py-4 text-center">
                                <x-button wire:click="edit_register({{ $datos_muestra->id_descrip_muestra }})">
                            ...
                            <x-td>
                                @if ($datos_muestra->estatus == 1)
                                    <x-button wire:click="estatus_register({{  $datos_muestra->id_descrip_muestra  }})">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round"
                                ...
                                @else
                                    <x-danger-button wire:click="estatus_register({{  $datos_muestra->id_descrip_muestra  }})">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round"
    ...
    <div class="p-5">
        {{ $datos_muestras->links() }}
    </div>

    <x-dialog-modal wire:model="new">
        <x-slot name='title'>
            <h2 class="text-center">Nuevo Descripción Muestra</h2>
        </x-slot>
        <x-slot name='content'>
    ...
    <x-dialog-modal wire:model="edit">
        <x-slot name='title'>
            <h2 class="text-center">Editar Descripción Muestra</h2>
        </x-slot>
        <x-slot name='content'>
