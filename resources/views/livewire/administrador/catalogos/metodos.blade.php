<div>
    ...
    <div class="flex gap-3 items-center dark:text-white mb-5 max-md:flex-col">
        <div class="flex flex-row w-full gap-2">
            ...
            <div class="flex flex-col w-full">
                ...
                <x-input wire:model.live="search" placeholder="(Nombre método)" class="w-full" />
            </div>
        </div>
                ...
                <tbody>
                    @foreach ($datos as $dato)
                        <tr
                            ...
                            <td class="px-6 py-4 text-center">
                                <x-button wire:click="edit_register({{ $dato->id_metodo }})">
                            ...
                            <x-td>
                                @if ($dato->estatus == 1)
                                    <x-button wire:click="estatus_register({{  $dato->id_metodo  }})">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round"
                                ...
                                @else
                                    <x-danger-button wire:click="estatus_register({{  $dato->id_metodo  }})">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                            stroke-linecap="round" stroke-linejoin="round"
    ...
    <x-dialog-modal wire:model="new">
        <x-slot name='title'>
            <h2 class="text-center">Nuevo Método</h2>
        </x-slot>
        <x-slot name='content'>
            <form wire:submit="new_form">
                <div>
                    <x-label>Nombre Método:</x-label>
    ...
    <x-dialog-modal wire:model="edit">
        <x-slot name='title'>
            <h2 class="text-center">Editar Método</h2>
        </x-slot>
        <x-slot name='content'>
