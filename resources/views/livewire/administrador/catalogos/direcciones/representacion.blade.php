<div>
    ...
    <div class="flex gap-3 items-center dark:text-white mb-5 max-md:flex-col">
        <div class="flex flex-row w-full gap-2">
            ...
            <div class="flex flex-col w-full">
                ...
                <x-input wire:model.live="search" placeholder="(Nombre de la zona de representación)" class="w-full" />
            </div>
        </div>
                    ...
                    @foreach ($zonas as $zona)
                        <tr
                            ...
                            <th scope="row"
                                ...
                                {{ $zona->id_zona_representacion }}
                            </th>
                            <td class="px-6 py-4 text-center">
                                {{ $zona->nombre_zona }}
                            </td>
                            <td class="px-6 py-4 text-center">
                                <x-button wire:click="estado_register({{ $zona->id_zona_representacion }})">
                            ...
                            <td class="px-6 py-4 text-center">
                                <x-button wire:click="edit_register({{ $zona->id_zona_representacion }})">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                        stroke-linecap="round" stroke-linejoin="round"
    ...
    <x-dialog-modal wire:model="estados">
        <x-slot name='title'>
            <h2 class="text-center">Estados Zona de Representación</h2>
        </x-slot>
        <x-slot name='content'>
            <x-table>
                <x-slot name='titles'>
                ...
                <x-slot name='content'>
                    @foreach ($estadoszonas as $estado)
                        <x-tr>
                            <x-td>{{ $estado->estado->nombre }}</x-td>
                        </x-tr>
                    @endforeach
                </x-slot>
            </x-table>
        ...
        <x-slot name='footer'>
            <x-danger-button wire:click="estado_cancel">Cerrar</x-danger-button>
        </x-slot>
    </x-dialog-modal>
</div>
